function windowBlockOut = SingleWindowBlockout(datastore, n, imageSize)

global numImages buildingSceneReducedRGB imageSizeI2

imageSizeFull = imageSize
% buildingSceneOriginal = imageDatastore(buildingDirOriginal);
% I1 = readimage(buildingSceneOriginal, 1);    

imageSetDatastore = datastore;

% Resize the image to make it a manageable filesize
img = readimage(imageSetDatastore, n);
img = imresize(img,[500 NaN]);
imgSizeReduced = size(img);

img_copy2 = img;

% add a threshold for adding a border to the panorama
thresh = 6;
img = padarray(img, [thresh/2 thresh/2], 0, 'both');
img_copy = img;

% Some gaussian filtering
...
% Edge thresholding using a few heuristics MEDHIN PLEASE EXPLAIN THIS SHIT
% "I did some stuff to get the edges" is a huge cop-out...
...
total_img = img_copy2;
outline = zeros(imgSizeReduced(1),imgSizeReduced(2));
...
total_img = imresize(total_img,[imageSizeFull(1) imageSizeFull(2)]);

% Fill in the areas of the windows highlighted previously
SE = strel('rectangle',[64,64]);
closeBW = imclose(outline,SE);
closeBW = imresize(closeBW,[imageSize(1) imageSize(2)]);    

% Save window blockout regions for IR functions
windowBlockOut = closeBW; % MAKE THIS GLOBAL FOR THE FUNCTION
